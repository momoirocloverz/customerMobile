<template>
    <div class="echrts-m-pro">
        ...
        <div id="profit" v-show="!show && !loading" class="echart-box linebdb mb10"></div>
        <van-loading v-show="loading" type="spinner" color="#E95A34" vertical>加载中...</van-loading>
        <div class="pt44 pb40 tc fs14" v-show="!loading && show">暂无相关信息...</div>
    </div>
</template>
        ...
        data() {
            return {
                ...
                options: {
                    ...
                    grid: [{top: "15%", right: "5%", left: "15%", bottom: "35%"}],
                    series: [
                        {
                            name: '收入',
                            type: 'bar',
        ...
        methods: {
            getData(val) {
                ...
                let params = {
                    ...
                    managerIds: this.formCopy.managerId
                }
            ...
            init() {
                ...
                let len = this.options.xAxis.data.length;
                if(len > 8 && len <= 16){
                    this.options.dataZoom[0].end = 80;
                }else if(len > 16){
                ...
                if(len <= 3){
                    ...
                    this.options.xAxis.axisLabel.rotate = 90
                }
                console.log(this.options.xAxis.data)
                console.log(this.options.dataZoom[0])
