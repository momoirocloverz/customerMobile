            ...
            <div class="form-div bg-fff pb30 pl15 pr15 pt20">
                <van-cell-group>
                    <van-field
                            ...
                            left-icon=" phoneIcon"
                            maxlength="128"
                            placeholder="请输入您注册的手机号码"></van-field>
                    <van-field
                            class="mt20"
...
<script>
    export default {
        ...
        mounted() {
            console.log(JSON.parse(sessionStorage.getItem('cusInfo')).username)
            this.form.mobile = JSON.parse(sessionStorage.getItem('cusInfo')).mobile
        },
        methods: {
            goBack() {
                this.$router.go(-1);
