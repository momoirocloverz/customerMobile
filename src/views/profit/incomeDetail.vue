<template>
    <div class="income-wrap-detail">
        ...
        <div class="content">
            <ul class="pt15" v-show="tableData.length > 0">
                <li class="pt15 pb15 pl15 pr15 fs14 bg-fff" v-for="(item,index) in tableData"
                    :key="index"
                    @click="to(item)"
                >
                    ...
                    <p class=" ">
                        <span class="co-999 min-w-label">单价</span>
                        <span class="min-w-conten">{{item.price | forPrice}}</span>
                    </p>
                </li>
            </ul>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.title = `收入明细 ${this.$route.query.createAt} ${this.$route.query.managerName? this.$route.query.managerName : ''}`
            this.getData();
        },
        methods: {
            ...mapMutations(['IS_SET']),
            ...
            getData() {
                ...
                this.$api.classTaskProfitInComeDetail(params).then(res => {
                    console.log(res)
